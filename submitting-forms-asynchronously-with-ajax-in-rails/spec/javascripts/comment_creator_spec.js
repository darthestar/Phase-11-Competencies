describe("CommentCreator", function() {
  ...
  describe("append", function() {
    it("adds the comment to the comment section", function() {
      var commentTitle = $("Cinematic Gold!").val();
      var commentBody = $("You have to see this.").val();
      var commentCreator = newCommentCreator(attributes, commentsSection)
      commentCreator.append();
      var incomingComments = $("<div>", { id: "comments"});

      expect(incomingComments.find("h3").text()).toBe("Cinematic Gold!");
      expect(incomingComments.find("p").text()).toBe("You have to see this.");
    });
  });
});
